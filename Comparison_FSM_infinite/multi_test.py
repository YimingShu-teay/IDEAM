...
from Comparison_FSM_infinite.sodm import sodm
from Comparison_FSM_infinite.sodm_params import sodm_params
...
def extract_files_with_pattern(directory, pattern):
    # 用于存储匹配的文件路径
    matching_files = []
...
def one_round(dir_given):
    ...
    for i in range(N_t):
        ...
        desired_group = sodm_decision_maker.Decision_Making(u0[0],1000.0,group_dict, ego_group,path_now,X0)
        ...
        if i == 200:
            ...
            progress_20 = s_m
            metrics_save(S_obs_record,initial_params,progress_20,progress_40,TTC_record,vel,acc,steer_record,lane_state_record,path_record,C_label_record,initial_vds,"200",round_=round_,comparison="Comparison_FSM_infinite")
        ...
        if i == 400:
            ...
            s_m,ey_m,epsi_m = find_frenet_coord(path_m, x_m, y_m, samples_m,X0_g)
            progress_40 = s_m 
            metrics_save(S_obs_record,initial_params,progress_20,progress_40,TTC_record,vel,acc,steer_record,lane_state_record,path_record,C_label_record,initial_vds,"400",round_=round_,comparison="Comparison_FSM_infinite")
            break
           
        
        ################################# MPC solve and total env propogate ############           

        oa, od, ovx, ovy, owz, oS, oey, oepsi = mpc_controller.iterative_linear_mpc_control_for_comparison(X0, oa, od, dt, GPR_vy, GPR_w, C_label,  X0_g, path_d, last_X, path_now, ego_group, path_ego, desired_group,vehicle_left,vehicle_centre,vehicle_right,path_dindex)
...
if __name__ == '__main__':
    directory = r'C:\Users\sym02\Desktop\Research\Extension\codes\decision_change_rear\file_save'
    pattern = r'_200'
    matching_files = extract_files_with_pattern(directory, pattern)
    

    with Pool(processes=10) as pool:
        results = pool.map(one_round, matching_files)
